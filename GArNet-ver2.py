...
# Minimize energy and calculate Rosetta Energy Units (REU) and HiSol Score.<FITNESS_FUNCTION>
	...
	def __init__(self,pose,task_pack_mut,now_generation_number,selective_pressure,initial_flag,intmsa_data,stp_pose,library_hisol_score,tmp_seq,batch_converter,embed_lib):# <FITNESS_FUNCTION>
		
		self.pose = pose
		self.batch = batch_converter
		self.embed_lib = embed_lib
		self.tmp = tmp_seq
		...
		if initial_flag == int(0):
			...
			if selective_pressure == "ESM":
				self.esm_score = self.ESM_SCORE(self.tmp,self.pose,stp_pose,self.batch,self.embed_lib)
	...
	def ESM_SCORE(self,tmp_seq,pose,stp_pose,batch_converter,embed_lib):
		#Extracting embedding vectors.
		tmp_seq_2 = []
		tmp_seq_2.append(('generation_seq',f'{tmp_seq}'))
		batch_labels, batch_strs, batch_tokens = batch_converter(tmp_seq_2)

		with torch.no_grad():
			results = model(batch_tokens, repr_layers=[33], return_contacts=False)

		embeddings = []
		for i, (_, seq) in enumerate(tmp_seq_2):
			token_representations = results["representations"][33][i]
			seq_len = len(seq)
			embedding = token_representations[1:seq_len+1].mean(0).numpy()
			embeddings.append(embedding)

		embedding = np.array(embeddings)

		#Calculate Euclidean distance of per library-embeddings to target vector(sequence per genetarion).
		distance_list = []
		for i in range(len(embed_lib)):
			gen_distance = np.linalg.norm(embedding - embed_lib[i])
			distance_list.append(gen_distance)

		gen_distance = min(distance_list)
		print(f"all_distance: {distance_list}")
		print(f"min_distance: {gen_distance}")

		scorefxn2 = ScoreFunction()
		scorefxn2 = get_fa_scorefxn()
		if scorefxn2(pose)-scorefxn2(stp_pose) > float(0):
			gen_distance = float(10000000)
			print("Destabilized !!!")
...
# Mutagenesis creates many child-generations and selects the next parent-generations and elite by score.
	...
	def __init__(self,random_mutation_percent,now_generation_number,recombination_mutation_percent_increasing,sample_number,random_mutation_incidence,parent_candidate_number,directory_name,chain_name,parent_sequence,parent_residue_number,intmsa_data,parent_pose,task_pack_mut,selective_pressure,initial_flag,stp_pose,library_hisol_score,parent_pdb,X_mutations_list_X,frequency_border_line,tournament_entry_number,percent_determine_generation,batch_converter,embed_lib):# <FITNESS_FUNCTION>
		...
		# During Phase.I.
			...
			if selective_pressure == "ESM":
				self.esm_scores_array = np.zeros(sample_number,float)# <FITNESS_FUNCTION>-(GOAL)
			...
			for v in range(sample_number):
				...
				self.mutation_pose,self.task_pack_mut,self.tmp = self.MUTATE_POSE(self.parent_pose,self.mutations,self.task_pack_mut)

				# _____(Calculate scores.)_____
					
				calculation_score = CALCULATION_SCORE(self.mutation_pose,self.task_pack_mut,now_generation_number,selective_pressure,initial_flag,intmsa_data,stp_pose,[
				],self.tmp,self.batch,self.lib)# <FITNESS_FUNCTION>
				...
				if selective_pressure == "ESM":
					self.mutation_esm_score = calculation_score.esm_score# <FITNESS_FUNCTION>-(GOAL)
				...
				if selective_pressure == "ESM":
					self.esm_scores_array[v] = self.mutation_esm_score# <FITNESS_FUNCTION>-(GOAL)
			...
			elif selective_pressure == "ESM":
				self.evaluation_scores_array = self.esm_scores_array# <FITNESS_FUNCTION>-(GOAL)
			else:
		...
		# Final mutagenesis in Phase.II.
			...
			self.tmp_mutation_pose,self.task_pack_mut,self.gen_seq = self.MUTATE_POSE(parent_pose,self.mutations_list_final,task_pack_mut)
...
#		max_mutation_number = float(len(parent_sequence))*float(mutation_rate)
	...
	# Mutagenesis.
		...
		mutated_sequence = "".join([mutation_pose.sequence()[i] for i in range(mutation_pose.size())])
			
		return mutation_pose,task_pack_mut,mutated_sequence
...
while len(sys.argv) > 1:
	...
	# No problem without (="output.log").
		...
		esm_flag = int(1)# <FITNESS_FUNTION>-(GOAL)
		total_pressure_count += int(1)
		del sys.argv[1]
	elif sys.argv[1] == "-LIB":
...
if esm_flag == int(1):
	selective_pressure_tag += "ES"# <FITNESS_FUNCTION>-(GOAL)
...
if esm_flag == int(1):
	selective_pressure_variety.append("ESM")# <FITNESS_FUNCTION>-(GOAL)
	#Make lists of seqs library.  
	for record in SeqIO.parse(fasta_file, "fasta"):
		sequences.append(str(record.seq))

	for j in range(len(sequences)):
		lib_seqs.append((f"sample{j+1}", sequences[j]))

	tmp_residue = "".join(stp_residue_from_pdb)

	#Loading of ESM models.
	model, alphabet = esm.pretrained.esm1b_t33_650M_UR50S()
	batch_converter = alphabet.get_batch_converter()
	model.eval()

	batch_labels, batch_strs, batch_tokens = batch_converter(lib_seqs)

	with torch.no_grad():
		results = model(batch_tokens, repr_layers=[33], return_contacts=False)

	embeddings = []
	for i, (_, seq) in enumerate(lib_seqs):
		token_representations = results["representations"][33][i]
		seq_len = len(seq)
		embedding = token_representations[1:seq_len+1].mean(0).numpy()
		embeddings.append(embedding)

	embed_lib = np.array(embeddings)
...
if skip_GAOptimizer_flag == int(0):# If skip, the next step is to create a mutation network.
	...
	if esm_flag == int(1):
		selective_pressure_number += 1
		selective_pressure_data += "ESM or"# <FITNESS_FUNCTION>-(GOAL)
	...
	for n in range(len(selective_pressure_list)):# Cycle ("Set number" * "the number of selective pressure") .
		...
		for o in range(generation_number):# Generation.
			...
			if tmp_generation_number == int(0):
				...
				initial_calculation_score = CALCULATION_SCORE(stp_pose_from_pdb,task_pack_mut,tmp_generation_number,tmp_selective_pressure,initial_flag,intmsa_data_2,stp_pose,library_hisol_score,tmp_residue,batch_converter,embed_lib)# <FITNESS_FUNCTION>
				# Energy minimize.
				...
				tmp_residue = "".join(tmp_input_sequence)
				...
				calculation_score_EM = CALCULATION_SCORE(tmp_input_pose,task_pack_mut,tmp_generation_number,tmp_selective_pressure,initial_flag,intmsa_data_2,stp_pose,library_hisol_score,tmp_residue,batch_converter,embed_lib)# <FITNESS_FUNCTION>
				# Energy minimize.
...
# Create multiple PDBs by introducing mutations and determine the best variant in terms of score.
	...
	# Introduce selected mutations at a time.
		...
		for select_mutations_pattern in range(len(final_select_mutations_list_list)):
			...
			before_final_mutate_set_EM = CALCULATION_SCORE(stp_pose_from_pdb,final_task_pack_mut,O_generation_number_switch_O,O_selective_pressure_switch_O,O_initial_flag_switch_O,tmp_intmsa_data_2,stp_pose,O_library_hisol_score_O,tmp_residue,batch_converter,embed_lib)# <FITNESS_FUNCTION>
			# Energy minimize.
			
			final_mutate_set = MUTATION(X_X,O_generation_number_switch_O,X_X,X_X,X_X,X_X,X__X,X__X,X__X,X__X,X__X,stp_pose_from_pdb,final_task_pack_mut,X__X,X_X,X__X,X__X,X__X,tmp_select_mutations,X_X,X_X,X_X,batch_converter,embed_lib)
			# Mutagenesis.
			...
			after_final_mutate_set_EM = CALCULATION_SCORE(final_mutation_pose,final_task_pack_mut,O_generation_number_switch_O,O_selective_pressure_switch_O,O_initial_flag_switch_O,tmp_intmsa_data_2,stp_pose,O_library_hisol_score_O,tmp_residue,batch_converter,embed_lib)# <FITNESS_FUNCTION>
